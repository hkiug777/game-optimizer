...
### v3.0 (最新)
- **清理临时文件增强**：新增 NVIDIA 着色器缓存（DXCache + GLCache）、DirectX 着色器缓存（D3DSCache）、Windows 更新下载缓存清理
- **新增游戏环境检查**（选项 10）：检查游戏模式、硬件加速 GPU 调度（HAGS）、虚拟内存、显卡驱动版本、电源计划、内存容量，只检查不修改，给出优化建议
- **系统状态增强**：新增游戏模式状态显示、GPU 性能状态（P-state）显示
- **后台进程清理增强**：新增 Microsoft Store、反馈中心、你的手机、搜索主机、小组件、照片应用、媒体播放器、开始菜单主机等进程

### v2.9
- **新增内存深度清理功能**：通过 Windows API（EmptyWorkingSet）清空所有进程工作集，释放被缓存占用的内存。实测可释放 6GB+，完全临时性，不修改系统。
- 一键优化中加入内存深度清理步骤。

### v2.8
- 新增火绒安全软件临时关闭功能。
- 新增触摸键盘和手写面板关闭（TabTip.exe + TabletInputService 服务）。
- 新增迅游加速器监控进程 XYServiceLink.exe 关闭。
- 电源计划改为卓越性能。
...
| 选项 | 功能 | 说明 |
|------|------|------|
| 1 | 一键游戏优化 | 内存深度清理 + 关闭后台进程 + 停止非必要服务 + 卓越性能电源计划 + 刷新DNS |
| 2 | 仅结束后台进程 | 只关闭后台进程，不停止服务 |
| 3 | 设置游戏高优先级 | 自动识别三角洲/CS2，设置High优先级并确认电源计划 |
| 4 | 清理系统临时文件 | 清理TEMP、Windows\Temp、缩略图缓存、NVIDIA/DirectX着色器缓存、Windows更新缓存 |
| 5 | 网络延迟优化 | 刷新DNS缓存 + 测试网关/DNS延迟 |
| 6 | 临时关闭后台服务 | 停止更新、索引、遥测、诊断、Xbox、SysMain等服务 |
| 7 | 退出火绒安全软件 | 临时关闭火绒进程和服务（火绒有自我保护，可能需手动退出） |
| 8 | 内存深度清理 | 通过Windows API清空进程工作集，释放被缓存占用的内存（实测可释放6GB+） |
| 9 | 查看系统状态 | CPU/内存/显卡温度/电源计划/网络延迟/进程占用/游戏模式/GPU性能状态 |
| 10 | 游戏环境检查 | 检查游戏模式/HAGS/虚拟内存/驱动版本/电源计划/内存容量，只检查不修改 |
...
- **系统应用类**：Microsoft Store、反馈中心、你的手机、搜索主机、小组件、照片应用、媒体播放器、开始菜单主机
